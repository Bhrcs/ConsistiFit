...
assert(!`${html}\n${app}\n${plans}\n${guides}`.toLowerCase().includes('supabase'));
assert(!`${html}\n${app}\n${plans}\n${guides}`.toLowerCase().includes('rankfit'));

const buttons = [...`${html}\n${app}\n${plans}\n${guides}`.matchAll(/<button\b([^>]*)>/gi)];
...
console.log(`Playable PWA smoke test passed: ${buttons.length} button templates, persistent progression, plan generation, and exercise guides enabled.`);
